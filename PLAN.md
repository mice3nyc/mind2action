...
> **현재 빌드: v0.7** (5/25 기록됨) — 손소장 26.0525 수정 5건 + 리포트 헤더·마무리 정리. 역할별 안전구간 테이블(그래프 경계선 + 조율 발동 통합), 컨설턴트 CM6 공통 3섹션, "코칭"→"조율" 라벨, 결과화면 총점·재시도 삭제, 상단 1줄·✉ 이메일·하단 카피라이트. 호칭 일원화(②)는 드라이브 콘텐츠 시트 수령 후 보류.
...
- 리포트 웹페이지 렌더링 (보험설계사 ✅, 관리자·코치 CM 변환 완료, 호칭 일원화 대기)
...
- **v0.8 (다음): 손소장 전체 CM 데이터 재변환** — `data/Archives/{코치,리더,컨설턴트}.xlsx`(전 시트 CM2~CM7, 호칭 일원화됨)로 cm_*.yaml 통째 재변환. 현재 CM2(코치·리더)+CM6공통만 반영, CM3·CM4·CM5·CM6·CM7 미반영 → 옛 호칭 잔존. 재변환 = 데이터+호칭(②) 동시 해소. 코치 1직군 파일럿 후 병렬
...
| 8 | 호칭 일원화 결과 xlsx 도착 후 cm_*.yaml 재변환 | 대기 (손소장님 수작업) |
