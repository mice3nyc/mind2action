...
- [x] 목록 컬럼: 고객사·상태·참여시작·참여종료·교육일·**인원(참여/예상 = 12/40, 예상 미설정 시 참여수만)**·링크복사·**[설문 결과](그 캠페인 필터로 점프)**·관리(수정·마감, `campaign-actions` gap 8px). 레이블·데이터 중앙정렬. (인원 컬럼 = 5/31 Phase 2, 커밋 `91638b2`)
- [x] 링크 복사 버튼 아래 `?g={code}` 표시 제거(복사해도 무의미) + 수정/마감 버튼 간격(`83b7f9d`) — 5/31 피터공 피드백
...
- [x] **진행 현황 대시보드** ✅ 5/31 라이브 — 관리자 [진행 현황] 탭 신설(캠페인 관리 / 진행 현황 / 결과 확인). 캠페인별 카드: 참여 N / 예상 M (진행률 %, 100% 초과 허용) + 참여기간 종료 D-day + 교육일 D-day. 진행중 먼저 → 종료 임박순 정렬. 예상 미설정이면 진행률 대신 참여 인원만.
  - **결정됨 (피터공)**: 참여 예상 인원 필드 추가 (`expected_count` int). 실제가 예상보다 적어도 많아도 무방(기준값일 뿐). `LandingPage` 게이팅과 무관, 대시보드 진행률 표시용.
  - **DB 마이그레이션 완료**: `alter table campaigns add column if not exists expected_count int;` — 피터공 Supabase SQL Editor 실행 완료. (RLS 변경 없음 — campaigns는 authenticated만, 새 컬럼은 기존 정책 그대로 커버. RPC `get_campaign_by_code`는 설문 진입용이라 expected_count 미포함 = 변경 불필요.)
  - **파일**: `CampaignDashboard.jsx` 신설, `AdminDashboard.jsx` 탭 추가, `CampaignManager.jsx`·`campaigns.js` expected_count 입력. CSS `dashboard-*`(흰 카드·점선 구분·핑크 진행바). 코드 `059f0cc` + CSS `3153334` 푸시·배포 완료.
...
- [x] **관리자 목록에 참여기간 종료 반영** ✅ 6/14 — Phase 2 참여기간 자동 차단은 LandingPage(응답자 진입)에만 있어, 종료일이 지나도 관리자 캠페인 관리 목록은 저장된 `status` 원값("진행중")만 표시하던 구멍을 메움. `CampaignManager.jsx`에 LandingPage와 동일한 `todayStr()`(로컬 YYYY-MM-DD, 사전식==시간순) + `isPastEnd(camp)`(period_end 있고 `today > period_end` — 종료일 당일은 진행, LandingPage `ended` 기준과 일치) 추가. (1) **참여 종료 셀**: isPastEnd면 빨강(`td-period-ended`, 굵게). (2) **상태 뱃지**: `status==='active'` 이면서 isPastEnd면 `진행중` 대신 빨강 `설문종료`(`campaign-status-ended`). 수동 [마감](status=closed)은 날짜 무관하게 기존 '마감' 뱃지 유지(즉시 오버라이드가 날짜보다 우선 — LandingPage와 같은 위계). 마감/재개 토글 버튼·DB·마이그레이션 변경 없음(순수 표시 파생). CSS `.campaign-status-ended`(연빨강 배경/#dc2626)·`.td-period-ended`. ⚠️ 클라 시계 기준(LandingPage와 동일 MVP 한계).
- [ ] **설문 ON/OFF 토글 UI 정리** (선택) — 마감/재개 버튼이 이미 토글(status active↔closed)이나 토글처럼 안 보임. 필요 시 ON/OFF 스위치 모양으로.
- [ ] **진행 현황(CampaignDashboard) 종료 반영** (후속 후보) — 같은 isPastEnd 기준을 진행 현황 탭에도 적용할지. 6/14 캠페인 관리 화면만 우선 처리, 피터공 확인 후 결정.
