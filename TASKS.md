...
### 3-2. 직무→리포트 매핑 (손소장 확인 대기)
- 영업 → 보험설계사 CM
- 코치/멘토 → 코치 CM
- 영업팀장/센터장, 지점장/지사장, 교육팀장/지원팀장, 사업단장/부장, 본부장 → 관리자 CM

### 3-3. CM 조회 엔진
- [ ] scoreResult → CM 텍스트 매칭 로직 (`cmLookup.js`)
- [ ] 점수구간 분류 함수 (17-20/14-16/11-13/8-10/0-7)
- [ ] CM4-3 조건부 판단 (코칭 불필요 구간 체크)
- [ ] CM4-4 트리거 조건 (CP/NP/A/FC 0-7 또는 AC 17+)

### 3-4. 리포트 렌더링
- [ ] 리포트 웹페이지 템플릿 (보험설계사 11p 먼저)
- [ ] 에고그램 바 차트 + 성공구간 비교 그래프
- [ ] 리포트 링크 생성 + 접근 제어
