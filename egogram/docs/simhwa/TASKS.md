...
- [x] **피터공 확인 항목 3건 승인** (2026-08-10) — ①로고까지 검정으로 간 표지 ②**소속 회색 유지** ③인쇄 윗여백 21mm 두 줄 분량. 셋 다 그대로 두는 것으로 확정하고 배포했다. ⚠️ **오각형 레이더 색은 안 건드렸다** — 요청이 "회사로고부터 세 줄까지"로 끊겨 있고 그 그림은 색이 곧 정보라서다. 손소장이 표지 전체를 뜻했다면 되돌아올 자리는 여기 하나이고, 그때 안은 **색 제거가 아니라 회색조 이동**(다섯 축을 구분은 하되 유채색을 뺀다).
...
### v0.22 — 손소장 9/17 수정요청 (2026-09-18, SPEC §23)
- [x] 새 4장 「고객의 성향을 10분 내에 파악하기 어려웠다면」 · 옛 4·5장 → 5·6장 · 인쇄 10쪽
- [x] 일괄 출력 `/simhwa-batch/:campaignId` + 캠페인 관리 버튼
- [x] 인쇄 하니스에 판정 추가(쪽수·4장 한 쪽) · 변이 시험 붉음 확인
- [x] 배포 · 라이브 (8ffb222 · index-DbKcEBdg.js)
- [ ] 일괄 출력 실캠페인 확인 — 관리자 로그인 필요. **사람 손 미확인**
