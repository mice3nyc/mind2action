...
> ⚠️ **2026-07-29 — 이 문서의 성향 이름·색 서술은 낡았다.** 두 가지가 바뀌었다.
> 1. **라벨 5종 교체**(손소장 확정, v0.13 `2462307`): `기준·결단→원칙·결단` / `배려·공감→공감·배려` / `협조·조율→순응·협조`. A·FC는 그대로. 아래 §에 남은 옛 표기는 이력으로 읽는다.
> 2. **"EGO_COLORS를 ResultPage에 로컬 복제"라는 패턴은 폐기됐다.** 이름·색이 9파일에 복사돼 있던 것을 단일 출처 사전으로 통합했다. 이제 어디서도 다시 정의하지 않으며, `npm run check:terms`가 재정의를 실패로 잡는다.
>
> **성향 이름·색의 정본은 `egogram/src/data/ego_terms.yaml`이고, 규칙은 `egogram/docs/SPEC-ego-terms.md`다.** 이 문서는 그 앞의 이력으로 남긴다.
...
### 테이블 정렬 (7/3 추가)
소속·직무·소득 헤더 클릭으로 정렬. 기본(정렬 없음)은 `created_at` 내림차순(최근 입력 순) — Supabase 쿼리 그대로.
- 헤더 클릭 1회 = 오름차순, 재클릭 = 내림차순, 다른 헤더 클릭 시 그 헤더로 전환(오름차순부터)
- **소속**: 문자열 `localeCompare('ko')`
- **직무**: `JOB_LABELS` 한글 라벨 기준 `localeCompare('ko')` (코드값 아님)
- **소득**: 구간 순서 배열 기준 인덱스 비교(사전순 아님 — `1000-1500`이 `200-400`보다 실제로 커야 함)
  ```
  INCOME_ORDER = [under200, 200-400, 400-600, 600-800, 800-1000, 1000-1500, 1500-2000, over2000]
  ```
- 정렬은 필터링된(`filtered`) 결과 위에서 적용, 삭제/CSV 다운로드 대상에는 영향 없음(현재 화면 뷰만)
- 소속·직무·소득 헤더는 버튼 형태(`.th-sort-btn`)로 렌더 — **기본 상태가 검정 fill**(배경 검정+흰 글자, 항상 버튼처럼 보임), **hover·활성 시 파란(`#0012de`) fill**로 전환. 비활성 화살표는 반투명 흰색 `⇅`, hover·활성 시 완전 흰색
- **캠페인 필터 드롭다운을 다시 선택하면 정렬은 디폴트(입력순)로 리셋된다** (7/3 추가 — 필터 바뀌면 이전 정렬 기준이 새 목록에 안 맞을 수 있어서)
- **"전체리포트 PDF 출력하기"도 결과 테이블 정렬을 따른다** (7/3 추가): 결과 테이블에서 정렬 중이면 `#/report-batch/{campaignId}?sortKey=&sortDir=` 쿼리스트링으로 전달, `ReportBatchPage`가 로드 후 같은 기준(`sortRows`, `lib/resultLabels.js`)으로 재정렬. 정렬 안 하고 있으면 기존과 동일하게 `created_at` 오름차순. 라벨·정렬 로직은 `lib/resultLabels.js`에 공용화(AdminDashboard 자체 정의 폐기 — 두 곳 따로 두면 라벨 어긋날 위험)
...
### 리포트 렌더링 변경 (v0.12 — 피터공 26.0613, 폭·줄바꿈 정리)
...
- **(1) 커버 소속·이름 한 줄**: `.report-cover-id`가 이름↵소속 2줄로 렌더되던 것 → `소속(company+department, 얇게 weight 300) + 이름(볼드 weight 700)` 인라인 한 줄. JSX: `report-cover-name`·`report-cover-meta`를 `div`→`span`, meta는 `[company, department].filter(Boolean).join(' ')`. 기존 `span+span::before{content:' · '}` 규칙 폐기(단일 span). CSS: name weight 400→700·margin 0, meta weight 300·14px·margin-right 10px
- **(2) §1 정체성 박스 줄간격**: `.report-identity-line`("OOO님은 ___한 분입니다") ↔ `.report-identity-desc`(설명) 사이 간격 과대 → `.report-identity-desc` margin-top 12px→4px
- **(3) §1 성향별 보기 한 줄**: 라벨(`이성·판단`)·점수와 한 줄 평이(`plainTranslation`)가 2줄로 갈리던 것 → `이성·판단 18점 : 강한 분석력…` 한 줄. JSX: `report-trait-ego`(div)+`report-trait-plain`(p) → 단일 `p.report-trait-ego` 안에 `report-trait-ego-name`(색·볼드)·`report-trait-ego-score`(점수)·`report-trait-ego-sep`(:)·`report-trait-ego-plain`(평이 600). CSS 특정성: `.report-trait-item .report-trait-ego`로 `.report-trait-item p{margin:0}` 위에 margin 0 0 8px 확보
...
| 「가장 강한 힘 / {top1 강점명}」 라벨 | ~~유지~~ → **삭제 (9/18 오후 손소장 메시지: 「가장 강한 힘을 삭제 부탁해요. 보폭을 맞추는 힘, 흐름을 읽는 힘 등」)**. 라벨 줄과 강점명 줄 둘 다 뺀다. 목업(ChatGPT 편집본)에 없던 것이 생략이 아니라 의도였다. 강점명(`EGO_TYPE_NAME`)은 다른 곳이 쓰므로 데이터는 두고 이 박스에서만 뺀다 |
...
```
(가장 강한 힘 / 강점명 두 줄 삭제 — 9/18 오후 손소장)
